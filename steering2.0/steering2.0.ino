...
void loop()
{
  ...
  if( 1550 < APMIN < 1850 && !left_turn )// turn left  
  {
    right_turn = false;
    left_turn = true;
    steer = 0;
  ...
  {
    if(left_turn)//if was previously left,turn right
    {
      steer = 1;
      left_turn = false;
    }
    if(right_turn)//if was previously right, turn left
    {
      steer = 0;
      right_turn = false;
    }
  }
  if( 1000 < APMIN < 1350 && !right_turn )
  {
    left_turn = false;
    right_turn = true;
    steer = 1;
